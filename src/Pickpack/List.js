  ...
  useEffect(() => {
    const getOrders = async () => {
      try {
        ...
        const url = `https://api2.ebazaar.mn/api/inventory/list/get?page=${currentPage}&limit=50`;
        ...
        <div className={css.content} id="scrollDiv">
          ...
          >
            {pickpackOrders.map((order) => {
              return <SingleOrder key={`pickpack-order-list-single-${order._id}`} order={order} products={products} setPickpackOrders={setPickpackOrders} />;
            })}
          </InfiniteScroll>
        </div>
      )}
...
const SingleOrder = ({ order, products, setPickpackOrders }) => {
  ...
  useEffect(() => {
    ...
    for (const prod of order.line) {
      const currentProd = products.find((product) => product._id === prod.product_id);
  ...
  const submitHandler = async () => {
		try {
			const url = `https://api2.ebazaar.mn/api/pickpack/orderstatus`;
