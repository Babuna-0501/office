    ...
    <div className={css.listContainer} id='scrollDiv'>
      ...
      >
        {orders.map((order, index) => {
          return (
            <SingleOrder
              key={`single-order-${order.order_id}`}
...
const SingleOrder = ({ zIndex, order, suppliers, locations, channels }) => {
  const [currentSupplier] = useState(
    suppliers.find(supplier => supplier.id === order.supplier_id)
  );
  const [currentCity] = useState(
    locations.find(
      location => location.location_id === Number(order.tradeshop_city)
    )
  );
  const [currentDistrict] = useState(
    locations.find(
      location => location.location_id === Number(order.tradeshop_district)
    )
  );
  const [currentKhoroo] = useState(
    locations.find(
      location => location.location_id === Number(order.tradeshop_horoo)
    )
  );
  const [currentChannel] = useState(
    channels.find(
      channel => channel.business_type_id === Number(order.business_type_id)
    )
  );
          ...
          <div className={css.singleOrderFieldWrapper} style={{ width: 150 }}>
            {order.line.slice(0, 4).map((product, index) => {
              return (
                ...
                >
                  <img
                    src={replaceImageUrl(product.product_image.split(',')[0])}
