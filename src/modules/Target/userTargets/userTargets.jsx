...
const UserTargets = () => {
  ...
  const getTargetByUser = async () => {
    try {
      let url = `${process.env.REACT_APP_API_URL2}/api/backoffice/users/targets?userId=${userId}`;
      if (startDate && endDate) {
        url = `${process.env.REACT_APP_API_URL2}/api/backoffice/users/targets?userId=${userId}&startDate=${startDate}&endDate=${endDate}`;
      }
  ...
  const getUser = async () => {
    try {
      const url = `${process.env.REACT_APP_API_URL2}/api/backoffice/users?id=${userId}`;
