...
export const ProductTargetAdd = (props) => {
  ...
  const getProducts = async () => {
    try {
      ...
      if (productName || productSupplier || productCategory || productBrand || productBarcode || productSku) {
        currentPageCopy = 1;
        setCurrentPage(1);
      }
      ...
      const url = `https://api2.ebazaar.mn/api/products/get1?${params}`;
      ...
      if (currentPageCopy === 1) {
        setProducts(resData.data.map((prod) => ({ ...prod, singlePrice: prod.locations?.["62f4aabe45a4e22552a3969f"]?.price?.channel?.[1] ?? 0 })));
      } else {
        setProducts((prev) => [...prev, ...resData.data.map((prod) => ({ ...prod, singlePrice: prod.locations?.["62f4aabe45a4e22552a3969f"]?.price?.channel?.[1] ?? 0 }))]);
      }
    } catch (error) {
  ...
  const getData = async () => {
    try {
      ...
      const [supplierRes, extraRes] = await Promise.all([fetch(suppliersUrl, requestOptions), fetch(suppExtraDataUrl, requestOptions)]);
  ...
  }, [currentPage, productName, productSupplier, productCategory, productBrand, productBarcode, productSku]);

  const saveHandler = () => {
    try {
      if (selectedProducts.length === 0) throw new Error("Бүтээгдэхүүн сонгоно уу!");
      if (selectedProducts.length === products.length && totalAmount === "") throw new Error("Багц бүтээгдэхүүний төлөвлөгөөний дүн оруулна уу!");
      if (selectedProducts.filter((prod) => prod.target).length !== selectedProducts.length && selectedProducts.length !== products.length)
        throw new Error("Сонгосон бүтээгдэхүүнд төлөвлөгөө оруулна уу!");

      if (selectedProducts.length === products.length) {
        if (categoryTargetExist || brandTargetExist) throw new Error("Ангилал болон брэнд төлөвлөгөө үүссэн тул бүтээгдэхүүн багц төлөвлөгөө үүсгэх боломжгүй!");
        ...
        setTarget((prev) => ({
          ...prev,
          products: selectedProducts.map((prod) => ({ _id: prod._id, target: { ...prod.target }, succeeded: { amount: 0, quantity: 0 }, waiting: { amount: 0, quantity: 0 } })),
          type: 1,
        }));
...
const SingleProduct = ({ product, zIndex, suppliers, categories, brands, selectedProducts, setSelectedProducts, products }) => {
  ...
  const [amount, setAmount] = useState(product.target ? product.target.amount : "");
  const [quantity, setQuantity] = useState(product.target ? product.target.quantity : "");

  useEffect(() => {
    if (amount) {
      setSelectedProducts((prev) => prev.map((prod) => (prod._id === product._id ? { ...prod, target: { amount: Number(amount), quantity: null } } : prod)));
    } else {
  ...
  useEffect(() => {
    if (quantity) {
      setSelectedProducts((prev) => prev.map((prod) => (prod._id === product._id ? { ...prod, target: { amount: null, quantity: Number(quantity) } } : prod)));
    } else {
      setSelectedProducts((prev) =>
        prev.map((prod) => {
          if (prod._id === product._id) {
  ...
  return (
    <div className={`${css.contentRow} ${checked && css.checked}`} style={{ zIndex }}>
      ...
      <div className={css.contentItem} style={{ width: 90 }}>
        <Input value={quantity} type="number" onChange={(e) => setQuantity(e.target.value)} disabled={!checked || amount || products.length === selectedProducts.length} size="small" placeholder="0" />
      </div>
    </div>
  );
};
