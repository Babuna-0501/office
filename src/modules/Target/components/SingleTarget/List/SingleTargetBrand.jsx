...
export const SingleTargetBrand = props => {
    ...
    initBrands
      .filter(
        brand =>
          ...
          [...new Set(brandTarget.map(target => target._id))].includes(
            brand.BrandID
          )
      )
      .map(brand => {
        const currentBrandTarget = brandTarget.find(
          target => target._id === brand.BrandID
        );
  ...
  useEffect(() => {
    ...
    if (brandName) {
      brandsCopy = brandsCopy.filter(
        brand =>
          brand.BrandName.toLowerCase()[0] === brandName.toLowerCase()[0] &&
          ...
          {brands.length > 0 && (
            <div className={css.tableContent}>
              {brands.map((brand, index) => {
                const currentMethod = brand.target.target.amount
                  ? 'amount'
                  : 'quantity';

                return (
                  <div
                    key={`product-target-${index}`}
                    className={css.tableRow}
                    style={{ zIndex: brands.length - index }}
                  >
                    ...
                    <div className={css.singleTableItem} style={{ width: 90 }}>
                      {brand.target.target.amount && (
                        <span className={css.targetText}>
                          {brand.target.target.amount.toLocaleString()}₮
                        </span>
                      )}
                    </div>

                    <div className={css.singleTableItem} style={{ width: 90 }}>
                      {brand.target.target.quantity && (
                        <span className={css.targetText}>
                          {brand.target.target.quantity.toLocaleString()}ш
                        </span>
                      )}
                    </div>
                      ...
                      <div className={css.targetProgress}>
                        ...
                        <div className={css.progressBar}>
                          <div
                            ...
                            style={{
                              width: `${Math.round(
                                (brand.target.succeeded[currentMethod] * 100) /
                                  brand.target.target[currentMethod]
                              )}%`
                            }}
                          />
                          <div
                            ...
                            style={{
                              ...
                              width: `${Math.round(
                                (brand.target.waiting[currentMethod] * 100) /
                                  brand.target.target[currentMethod]
                              )}%`
                            }}
                          />
