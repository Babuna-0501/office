...
export const UserCard = ({ target, user, userRoles, products, index }) => {
  ...
  useEffect(() => {
    if (hasTarget) {
      ...
      setTotalCompletedPercentage(totalCompleted === 0 ? 0 : Math.round((totalCompleted * 100) / totalTargetCopy));
      setTotalPendingPercentage(totalPending === 0 ? 0 : Math.round((totalPending * 100) / totalTargetCopy));
    }
