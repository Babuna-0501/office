...
const Login = () => {
  ...
  const submitHandler = () => {
    ...
    // Send login data
    setLoading(true);
    fetch(`https://api2.ebazaar.mn/api/login?email=${email}&password=${password}`)
      .then((r) => r.json())
      .then((response) => {
        if (response.code === 404) {
          ...
          setMainErrMsg("Холболтонд алдаа гарлаа!, Та түр хүлээгээд дахин оролдоно уу ...");
          setTimeout(() => {
            setMainErrMsg("");
          }, 2000);
          setLoading(false);
          return;
        } else if (response.code === 200 && response.message === "login successful") {
          setLoading(false);
  ...
  return (
    <div className={css.container} style={{ backgroundImage: `url(${bgImage})` }}>
      <div className={css.loginFormContainer}>
        <div className={css.formHeader} style={{ marginBottom: loading ? 0 : 47 }}>
        ...
        {!loading && (
          ...
          >
            <div style={{ marginBottom: emailErrMsg ? 0 : 20 }}>
              <Input value={email} onChange={(e) => setEmail(e.target.value)} size="large" placeholder="Имэйл" type="email" errorMsg={emailErrMsg} />
            </div>
