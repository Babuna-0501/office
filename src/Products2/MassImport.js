...
const MassImport = props => {
  ...
  const save = () => {
    ...
    const hasHighlightedBarcodes = rows.some(row =>
      supProducts.some(e => e.bar_code === row?.barcode)
    );
  
    if (hasHighlightedBarcodes) {
      alert('Энэ баркод давцсан байна. Шинээр солино уу');
      return; 
    }
          ...
          ? [
              {
                ...
                resource: product.resource ?? null
              }
            ]
          : [],
        locations: {
        ...
        <div className='container'>
          ...
          <div id='rows'>
            ...
            {loading ? (
              ...
              <div style={{ width: '100%', maxHeight: '75%', overflowY: 'scroll' }}>
                {rows.map((row, index) => {
                  const isBarcodeHighlighted = supProducts?.some(e => e.bar_code === row?.barcode);
                    ...
                    >
                      {[
                        ...
                        .map(field => {
                          ...
                          if (field === 'city_tax' && row?.category === 'Алкоголь') {
                            displayValue = 1;
                          } else if (
                            ['stock', 'price', 'wholePrice', 'unitPrice'].includes(field)
                          ) {
                          ...
                          return (
                            <div
                              key={field}
                              style={{
                                color: field === 'barcode' && isBarcodeHighlighted ? 'red' : 'black',
                                fontWeight: field === 'barcode' && isBarcodeHighlighted ? '700' : 'normal',
                              }}
                            >
                              {displayValue}
                            </div>
                      ...
                      {isBarcodeHighlighted && (
                        <div>
                          <img
                            ...
                            alt='delete'
                            onClick={() =>
                              setRows(prev => prev.filter(q => q.barcode !== row?.barcode))
                            }
                          />
                        </div>
                      )}
