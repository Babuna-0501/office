...
const List = ({
  ...
  const getOrders = async (filter, loading = true, p) => {
    ...
    console.log(localStorage.getItem('ebazaar_admin_token'));
    //  if (orderPrice) {
    ...
    if (filterState.payment_status) {
      changeParams(filterState.payment_status, 'payment_status');
    }
    ...
    url = admin
      ? `${process.env.REACT_APP_API_URL2}/api/orders/?${params}page=${p - 1}`
      : `${process.env.REACT_APP_API_URL2}/api/order/${
          suppliers ? 'sfa' : 'b2b'
        }?${params}page=${p}`;
  ...
  const fetchData = (filter, loading = true) => {
    ...
    if (loading) setLoading(true);
    const url = admin
      ? `${process.env.REACT_APP_API_URL2}/api/orders/?order_type=1&page=${
          page - 1
        }`
      : `${process.env.REACT_APP_API_URL2}/api/order/${
          suppliers ? 'sfa' : 'b2b'
    ...
    <>
      <div
        ...
        onScroll={e => {
          ...
          if (bottom && filteredData.length > 0 && scrollTop > 0) {
            setPage(prev => prev + 1);
          }
        }}
      >
