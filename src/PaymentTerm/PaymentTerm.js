...
const PaymentTerm = props => {
  ...
  useEffect(() => {
    ...
    fetch(
      `${process.env.REACT_APP_API_URL2}/api/products/get1?ids=[${checkedProd}]`,
      requestOptions
    )
      .then(res => res.json())
      .then(res => {
  ...
  const fetchProduct = () => {
    ...
    let url = `${process.env.REACT_APP_API_URL2}/api/products/get1?supplier=${companyId}`;
  ...
  const save = () => {
    ...
    if (activeSetting === 2) {
      ...
      fetch(
        `${process.env.REACT_APP_API_URL2}/api/product/channel/set`,
        requestOptions
      )
        .then(r => r.json())
        .then(res => {
