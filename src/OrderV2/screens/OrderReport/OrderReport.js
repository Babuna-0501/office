...
const OrderReport = () => {
  ...
  const fetchData = async () => {
    ...
    const apiUrl = `${process.env.REACT_APP_API_URL2}/api/order/tailan?start=${startDate}&end=${endDate}`;
    try {
  ...
  const handleExport = () => {
    ...
    const dataset = data.map(item => {
      ...
      return {
        ...
        price: item.price,
        total: item.total,
        orderAt: new Date(item.orderAt).toISOString().split('T')[0],
        deliveryAt: new Date(item.deliveryAt).toISOString().split('T')[0],
        phone: item.phone,
        ...
        recievedAt: item.recievedAt
          ? new Date(item.recievedAt).toISOString().split('T')[0]
          : '',
        brand: item.brand,
