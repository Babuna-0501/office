...
const ReportDetail = (props) => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      ...
      try {
        const response = await fetch("https://api2.ebazaar.mn/api/backoffice/suppliers", requestOptions);
  ...
  const fetchData = async () => {
    ...
    const apiUrl = `https://api2.ebazaar.mn/api/order/tailan/huraangui?startDate=${startDate}&endDate=${endDate}&supplierId=${suppId}`;
    try {
  ...
  const handleSearchChange = (e) => {
    ...
    const filtered = suppliers.filter(supplier => supplier.name.toLowerCase().includes(e.target.value.toLowerCase()));
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{display:"flex", flexDirection:"column"}}>
              ...
              {filteredSuppliers.length > 0 && (
                <ul className='brief_supp_ul' style={{border: '1px solid #ccc', maxHeight: '150px', overflowY: 'scroll'}}>
                  {filteredSuppliers.map(supplier => (
                    <li key={supplier.id} onClick={() => handleSupplierSelect(supplier.id, supplier.name)}>
                      {supplier.name}
                    </li>
                  ))}
                </ul>
