...
const Form = props => {
  ...
  const widths = [
    60, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120,
    120, 120, 120, 120, 120
  ];
  let width = 0;
    ...
    temp.map((product, index) => {
      if (product.uid === uid) {
        ...
        document.getElementById('cost' + uid).innerText =
          Number(e.target.value) * Number(temp[index].cost);
      }
    });
  };
  const cost = (e, uid) => {
      ...
      if (product.series) {
        product.series.map(serial => {
          availableSeries.push(
            <option value={serial._id}>{serial.seriesNumber}</option>
          );
        });
      }
      returnHTML.push(
        <div className='box_container' key={Math.random()}>
          ...
          <div className='box_header' style={{ width: widths[7] }}>
            <input
              ...
              style={{ height: '30px' }}
              defaultValue={
                product.sellPrice && product.sellPrice.wholesale
                  ? product.sellPrice.wholesale
                  : null
              }
              onKeyUp={e => wholesale(e, product.uid)}
            />
          </div>
          <div className='box_header' style={{ width: widths[8] }}>
            <input
              ...
              onKeyUp={e => quantity(e, product.uid)}
              defaultValue={product.quantity}
            />
          </div>
          <div className='box_header' style={{ width: widths[9] }}>
            <p id={'cost' + product.uid}></p>
          </div>
          ...
          <div className='box_header' style={{ width: widths[12] }}>
            <span onClick={() => removeProduct(index)}>
              <img
                src='https://admin.ebazaar.mn/images/remove.svg'
                alt=''
                style={{ width: '26px' }}
              />
            </span>
  ...
  const addProducts = selectedProducts => {
    ...
    selectedProducts.map(selectedProduct => {
      ...
      let url = `${process.env.REACT_APP_API_URL2}/api/warehouse_series/get?productId=${selectedProduct._id}&warehouseId=${context.activeWarehouse._id}`;
  ...
  const renderHTMLWarehouses = context.warehouseList.map(warehouse => {
    return context.activeWarehouse._id !== warehouse._id ? (
      <option key={Math.random()} value={warehouse._id}>
        {warehouse.name}
      </option>
    ) : null;
  });
  ...
  for (const outboundType in outboundTypes) {
    renderHTMLOutboundTypes.push(
      <option key={Math.random()} value={outboundType}>
        {outboundTypes[outboundType]}
      </option>
    );
  }
  const send = () => {
  ...
  if (props.form[0] === 'new') {
    renderButtons = (
      <>
        ...
        <button onClick={() => send()} className='button primary large'>
          Илгээх
        </button>
      </>
    );
        ...
        <div id='overlaypage'>
          <div className='left'>
            ...
            <div>
              ...
              <div style={{ width: '320px' }}>
                ...
                <div>
                  <select
                    ...
                    className='custom-select'
                    style={{ width: '300px', margin: '0 1rem 0 0' }}
                    value={props.form.from ? props.form.from : sendingWarehouse}
              ...
              <div style={{ width: '320px' }}>
                ...
                <div>
                  <select
                    ...
                    value={outboundType}
                    style={{ width: '300px', margin: '0 1rem 0 0' }}
                    value={props.form.type ? props.form.type : null}
              ...
              <div style={{ width: '320px' }}>
                <textarea placeholder='Тэмдэглэл' id='requestNote'>
