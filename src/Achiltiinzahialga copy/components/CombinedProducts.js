...
export const CombinedProducts = (props) => {
  ...
  useEffect(() => {
    const getShipments = async () => {
      try {
				...
				// } else {
				// 	let url = `https://api2.ebazaar.mn/api/shipment?supplierId=${companyId}&startDate=${startDate}&endDate=${endDate}&page=0`;
				// }
  ...
  return (
    <div onClick={closeHandler} className={css.printRecieptContainer} style={{ justifyContent: products.length >= 17 ? "flex-start" : "center" }}>
      <div onClick={(e) => e.stopPropagation()} ref={receiptRef} className={css.receiptContainer}>
            ...
            <>
                  ...
                  {products.map((product, index) => {
                    return (
                      <tr key={`receipt-${product._id}`}>
                        ...
                        <td>{product.locations?.[`62f4aabe45a4e22552a3969f`]?.price?.channel?.[1].toLocaleString()}₮</td>
                        <td>{product.count}</td>
                        <td>{(product.locations?.[`62f4aabe45a4e22552a3969f`]?.price?.channel?.[1] * product.count).toLocaleString()}₮</td>
                      </tr>
                    );
                  })}
                </tbody>
              ...
              <div className={css.productFooterWrapper}>
                <div className={css.productsDetail}>
                  ...
                  <span>Нийт бүтээгдэхүүн: {products.reduce((acc, cur) => acc + cur.count, 0).toLocaleString()}ш</span>
                  <div>Нийт үнийн дүн: {products.reduce((acc, cur) => acc + cur.count * cur.locations?.[`62f4aabe45a4e22552a3969f`]?.price?.channel?.[1], 0).toLocaleString()}₮</div>
                </div>

                <div className={css.infoFooter}>
                  <div className={css.rightSide}>
                    <span>Ачилтын нэгтгэл үүсгэсэн:&nbsp; ________________________</span>
                    <span>Ачилтын хүлээн авсан: &nbsp;&nbsp;&nbsp; &nbsp;&nbsp; ________________________</span>
                    <span>Ачилтыг хянасан: &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp; &nbsp;&nbsp; ________________________</span>
                  </div>
