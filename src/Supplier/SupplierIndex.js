...
const SupplierIndex = props => {
	...
	useEffect(() => {
		const fetchdata = async () => {
			try {
				...
				if (search) {
					url = `https://api2.ebazaar.mn/api/backoffice/suppliers?id=${search}`;
				}
	...
	useEffect(() => {
		...
		let url = `https://api2.ebazaar.mn/api/merchants?id=${coMerchant.join(
			","
		)}`;

		if (merSearch) {
			url = `https://api2.ebazaar.mn/api/merchants?id=${merSearch}`;
		}
