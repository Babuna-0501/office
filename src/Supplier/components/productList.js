...
const ProductList = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let url = `${process.env.REACT_APP_API_URL2}/api/products/get1?search=${search}&suppliers=${supplierIds}`;
      if (searchSupplier === 'All' || searchSupplier === '') {
        url = `${process.env.REACT_APP_API_URL2}/api/products/get1?search=${search}&suppliers=${supplierIds}`;
      } else {
        url = `${process.env.REACT_APP_API_URL2}/api/products/get1?search=${search}&suppliers=${searchSupplier}`;
      }
          ...
          <div className={css.merchantList} style={{ overflow: 'scroll' }}>
            {merchants.map((merchant, index) => (
              <div
                ...
                style={{
                  backgroundColor: merchantId == merchant.value ? '#cfd8dc' : ''
                }}
                onClick={() => {
