...
const Wrapper = props => {
  ...
  const SendTwoHandler = () => {
    ...
    fetch(
      `${process.env.REACT_APP_API_URL2}/api/orders?id=${props.item.order_id}`,
      requestOptions
    )
      .then(res => res.json())
      .then(res => {
        ...
        if (approve) {
          ...
          let registerCheckUrl = `https://api2.ebazaar.link/api/ebarimt/merchant/info?regno=${props.item.register.toUpperCase()}`;
            ...
            .then(res => {
              ...
              if (res.found == true) {
                ...
                props.item.line.map(item => {
                  ...
                  if (props.item.coupon_amount) {
                    // MAIN COUPON LOGIC discount=product/total order*coupon
                    x = x - (x / totalAmountOrig) * props.item.coupon_amount; //tempVat += x/
                    if (item.alcohol) {
                      _city += x / 56;
                      _total += x;
                ...
                let newUrl = `${process.env.REACT_APP_API_URL2}/api/ebarimt/barimt`;
  ...
  if (props.item.coupon_amount) {
    ...
    const percentage = (
      (props.item.coupon_amount * 100) /
      totalAmountOrig
    ).toFixed(2);
    console.log(
      '----------' + (totalAmountOrig - (totalAmountOrig / 100) * percentage)
    );
  }
  return (
    <div
    ...
    >
      <div>
        ...
        <div
          style={{
            margin: '1rem 0',
            background: props.item.coupon_amount ? 'brown' : '#ccc',
            borderRadius: '3px',
            padding: '0 .25rem'
          }}
        >
          <span
            style={{
              fontSize: '11px',
              color: props.item.coupon_amount ? 'white' : ''
            }}
          >
            Coupon:{' '}
            {props.item.coupon_amount
              ? props.item.coupon_amount
              : '--------------------'}
          </span>
        </div>
          ...
          <div className={css.bothhhh}>
            {props.item.line.map((x, index) => {
              ...
              if (props.item.coupon_amount) {
                ...
                console.log(
                  '---------- --- totalAmountOrig ' + totalAmountOrig
                );
                console.log(
                  '---------- --- coupon ' + props.item.coupon_amount
                );
                console.log(totalPrice);
                totalPrice =
                  totalPrice -
                  (totalPrice / totalAmountOrig) * props.item.coupon_amount;
                console.log(totalPrice);
              }
              //
