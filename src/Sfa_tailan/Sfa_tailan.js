...
const SfaTailan = (props) => {
  ...
  useEffect(() => {
    fetch(
      `https://api2.ebazaar.mn/api/backoffice/users?company=${userData?.company_id}`,
      requestOptions
    )
      .then((res) => res.json())
  ...
  useEffect(() => {
    if (datas.length > 0) {
      ...
      const newData = datas.reduce(
        (result, { tradeshop_id, back_office_user, grand_total, tradeshop_name }) => {
          const existingEntry = result.find((entry) => entry.tradeId === tradeshop_id);
          ...
          if (existingEntry) {
            ...
            if (existingBackUser) {
              ...
              existingEntry.backOff.push({ backId: back_office_user, amount: grand_total });
            }
          } else {
            result.push({
      ...
      datas.map((data) => {
        ...
        if (backOffUserIds.length > 0) {
          ...
          backOffUserIds.forEach((userId) => {
            const existBackOffUser2 = backOffUserAmounts.find((item) => item && item.id === userId);
          ...
          <div className={`${css.chartLeft}  ${css.chartParentColumn}`}>
            <div className={css.chartPart}>
              ...
              <div className={css.chartMerchant}>
                <BarChart backOffData={backOffData} backOffUsers={backOffUsers} />
              </div>
            </div>
          ...
          <div className={`${css.chartMiddleRight}  ${css.chartParentRow}`}>
            ...
            <div className={`${css.chartRight}  ${css.chartParentColumn}`}>
              <div className={css.chartPart}>
                ...
                >
                  <ColumnChartTradeShop tradeData={tradeData} backOffUsers={backOffUsers} />
                </div>
              </div>
