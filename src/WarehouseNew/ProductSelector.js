...
const ProductSelector = (props) => {
			...
			if(show) {
				renderHTML.push(
					<div className="box_container" style={{width: width}}>
						<div className="box" style={{width: `${widths[5]}px`}}><button className="pageButton small" style={{padding: '0 .5rem', fontSize: '.75rem', background: title === 'Нэмэх' ? '' : '#ccc'}} onClick={() => fn(product)}>{title}</button></div>
	...
	selectedProducts.map(product => {
		renderHTMLSelectedProducts.push(
			<div className="box_container" style={{width: width}} key={Math.random()}>
			...
			<div id="container_productselector">
				<div id="productselector">
					...
					<div className="right">
						...
						<div className="selectedproducts" style={{position: 'absolute', top: '120px', right: '0px', bottom: '100px', left: '0'}}>
							{selectedProducts.length === 0 ? <p>Бүтээгдэхүүний жагсаалтаас сонгож оруулна уу.</p> : renderHTMLSelectedProducts}
						</div>
						<div className="bottom">
							<button onClick={() => props.setProductSelector(false)} class="button secondary large" style={{background: 'white'}}>Цуцлах</button>
							<button class="button primary large" onClick={() => props.addProducts(selectedProducts)}>Нэмэх</button>
						</div>
					</div>
