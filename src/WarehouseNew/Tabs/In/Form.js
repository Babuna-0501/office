...
const Form = props => {
  ...
  const addProducts = selectedProducts => {
    selectedProducts.forEach(product => {
        ...
        console.log(product)
        const isSupplierId14010 = props.data.some(item => item.supplierId === 14010);
        let subCategoryName = '';
        let formName = '';

        if (isSupplierId14010) {
          if (product.attributes && product.attributes.length > 0) {
              const subCategoryIndex = product.attributes[0].subCategory;
              subCategoryName = subCategory[subCategoryIndex] || '';
      
              const formIndex = product.attributes[0].form;
              formName = emHanganForm[formIndex] || '';
          } else {
              subCategoryName = '';
              formName = '';
          }
        ...
        <div class="blah productEntry" id="${uid}" data-id="${product._id}">
            ...
            <div class="blahblah">
                <div class="width120px">
                    ${product.image && product.image[0]
                        ? `<img src=${replaceImageUrl(product.image[0])} class="width40px" />`
                        : ''
                    }
                </div>
            </div>
            ...
            <div class="blahblah">
                <div class="width120px">
                    <input type="text" id="totalCost${uid}" readonly />
                </div>
            </div>
            <div class="blahblah">
                <div class="width120px">
    ...
    `;
        ...
        document.getElementById(`quantity${uid}`).addEventListener('input', updateTotalCost);
        document.getElementById(`cost${uid}`).addEventListener('input', updateTotalCost);

        function updateTotalCost() {
            const quantity = parseFloat(document.getElementById(`quantity${uid}`).value) || 0;
            const cost = parseFloat(document.getElementById(`cost${uid}`).value) || 0;
            const totalCost = quantity * cost;
            document.getElementById(`totalCost${uid}`).value = totalCost.toFixed(2);
        }
...
const send = async () => {
  try {
      ...
      productEntries.forEach(product => {
          ...
          const cost = parseInt(product.querySelector(`#cost${uid}`).value) || 0;
          const sellPriceRetail = parseInt(product.querySelector(`#sellprice${uid}`).value);
          const seriesNumber = product.querySelector(`#series${uid}`).value;
          // let __temp = {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL2}/api/shipment`, {
          method: 'POST',
          headers: myHeaders,
          redirect: 'follow',
      ...
      console.log(result);
      if (result.message === 'Амжилттай' || 'success') {
      ...
      alert('Алдаа гарлаа. Дахин оролдож үзнэ үү.');
  } finally {
      setSaving(false);
  }
};
  ...
  fetch(url, requestOptions)
    ...
    .then((response) => {
      if (response.message === 'success') {
        ...
        alert(response.message || 'Үлдэгдэл хүрэлцэхгүй байна!');
        setSaving(false); 
      }
    })
    .catch((error) => {
