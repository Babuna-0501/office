...
const Form = (props) => {
	...
	const [inboundType, setInboundType] = useState(props.form.variety ? props.form.variety : '')
	const sequenceSizes = {
		index: 80,
		id: 120,
		image: 120,
	...
	if(products.length > 0) {
		products.map((temp, index) => {
			...
			returnHTML.push(
				<div className="box_container" key={Math.random()} style={{width: width}}>
					...
					<div className="box_header" style={{width: widths[4]}}><p>{product.category}</p></div>
					<div className="box_header" style={{width: widths[5]}}><p>{product.bar_code}</p></div>
					<div className="box_header" style={{width: widths[5]}}>
						{
							formsHTML
						}
					</div>
					<div className="box_header" style={{width: widths[6]}}><p>{product.sku}</p></div>
					<div className="box_header" style={{width: widths[7]}}>
						<input type="number" style={{height: '30px'}} defaultValue={product.quantity} id={'product_quantity_' + product._id} key={Math.random()} />
					</div>
					<div className="box_header" style={{width: widths[8]}}>
						<input type="text" style={{height: '30px'}}  id={'cost' + product._id} onKeyUp={(e) => cost(e, product.uid)} defaultValue={product.cost} />
					</div>
					<div className="box_header" style={{width: widths[9]}}>
						<p>{product.cost * product.quantity}</p>
					</div>
					<div className="box_header" style={{width: widths[9]}}>
						<input type="text" style={{height: '30px'}}  id={'discount' + product._id} onKeyUp={(e) => discount(e, product.uid)} defaultValue={product.discount ? product.discount : ''} />
					</div>
					<div className="box_header" style={{width: widths[9]}}>
						<input type="text" style={{height: '30px'}}  id={'discount' + product._id} onKeyUp={(e) => discount(e, product.uid)} defaultValue={product.discount ? product.discount : ''} />
					</div>
					...
					<div className="box_header" style={{width: widths[11]}}><input type="text" style={{height: '30px'}} defaultValue={product.sellPrice ? product.sellPrice.retail : null} onKeyUp={(e) => retail(e, product.uid)} />
					</div>
					<div className="box_header" style={{width: widths[12]}}>
						<input type="text" style={{height: '30px'}} defaultValue={product.seriesNumber} onKeyUp={(e) => series(e, product.uid)} />
					</div>
					<div className="box_header" style={{width: widths[12]}}>
						<button className="pageButton small" onClick={() => duplicate(product)}>+ Сери</button>
					</div>
	...
	const renderHTMLWarehouses = context.warehouseList.map((warehouse) => {
		return  warehouse._id !== context.activeWarehouse._id ? <option key={Math.random()} value={warehouse._id} style={{fontWeight: context.activeWarehouse._id === warehouse._id ? 'bold' : 'normal'}}>{warehouse.name}</option> : null
	})
	...
	for (const inboundType in inboundTypes) {
		renderHTMLInboundTypes.push(<option key={Math.random()} value={inboundType}>{inboundTypes[inboundType]}</option>)
	}
	const send = () => {
		if(saving) {
		...
		const borderColor = document.getElementById('sendingWarehouse').style.borderColor
		if(type !== 'import' && document.getElementById('sendingWarehouse').value === '') {
			document.getElementById('sendingWarehouse').style.borderColor = 'red'
			setTimeout(() => {
				document.getElementById('sendingWarehouse').style.borderColor = borderColor
			}, 2000)
			return
		...
		products.map(product => {
			foo.push({
				...
				quantity: parseInt(document.getElementById('product_quantity_' + product._id).value),
	...
	if(props.form[0] === 'new') {
		renderButtons = (
			<>
				<button onClick={() => console.log(from + ' and  ' + inboundType)} style={{margin: '0 3rem 0 0'}}>Print</button>
				<button onClick={() => props.setForm(false)} className="pageButton secondary" style={{margin: '0 1rem 0 0!important'}}>Хаах</button>
		...
		if(props.form.status === 2) {
			renderButtons = (
				<>
					<button onClick={() => console.log(from + ' and  ' + inboundType)} style={{margin: '0 3rem 0 0'}}>Print</button>
					<button onClick={() => props.setForm(false)} className="pageButton" style={{margin: '0 1rem 0 0!important'}}>Засах</button>
					<button onClick={() => props.setForm(false)} className="pageButton secondary" style={{margin: '0 1rem 0 0!important'}}>Хаах</button>
				</>
			)
		} else {
	...
	const foobar = props.form[1] === 'customer' || props.form[1] === 'warehouse' ? (
		<div style={{width: '320px'}}>
			<div>
				<h2>{type === 'warehouse' ? 'Татан авах хүсэлт илгээх агуулах:' : 'Татан авах хүсэлт илгээх харилцагч:'}</h2>
			</div>
			<div>
				<select id="sendingWarehouse" className="custom-select" style={{width: '300px', margin: '0 1rem 0 0'}} defaultValue={from}>
				...
				<div id="overlaypage">
					<div id="overlaypage_header">
						...
						<div style={{display: 'flex', width: '1280px'}}>
							...
							<div style={{width: '320px'}}>
								...
								<div>
									<select id="inboundTypes" className="custom-select" style={{width: '300px', margin: '0 1rem 0 0'}} defaultValue={inboundType}>
										<option value="">--сонгоно уу--</option>
										{
							...
							<div style={{width: '500px'}}>
								...
								<textarea placeholder="Тэмдэглэл" id="requestNote" style={{width: '100%'}}>{props.form.note ? props.form.note : ''}</textarea>
							</div>
						</div>
						<div>
					...
					<div id="overlaypage_body" style={{overflow: 'auto'}}>
						<div className="box_header_container" style={{width: width}}>
							...
							<div className="box_header" style={{width: widths[15]}}>Хэлбэр</div>
							<div className="box_header" style={{width: widths[6]}}>SKU</div>
							<div className="box_header" style={{width: widths[7]}}>Татах тоо</div>
							<div className="box_header" style={{width: widths[8]}}>Өртөг</div>
							<div className="box_header" style={{width: widths[9]}}>Нийт өртөг</div>
							<div className="box_header" style={{width: widths[10]}}>Хямдралын хувь</div>
							<div className="box_header" style={{width: widths[10]}}>Хямдралын дүн</div>
							<div className="box_header" style={{width: widths[10]}}>Төлөх дүн</div>
							<div className="box_header" style={{width: widths[11]}}>Зарах үнэ</div>
							<div className="box_header" style={{width: widths[12]}}>Сери</div>
							<div className="box_header" style={{width: widths[12]}}>Сери +</div>
							<div className="box_header" style={{width: widths[13]}}>Дуусах</div>
