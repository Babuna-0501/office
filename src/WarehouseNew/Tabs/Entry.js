...
const Entry = (props) => {
	...
	if (boxQuantity && boxQuantity !== null) {
		totalBox = data.stock && parseFloat(data.stock) > 0.0 ? (data.stock / boxQuantity).toFixed(2) : ''
		if (totalBox % 1 === 0) {
			totalBox = parseInt(totalBox)
		}
	}
	let totalBoxCalculated = boxQuantity && boxQuantity !== null ? (data.reservedStock > 0 ? (data.reservedStock / boxQuantity).toFixed(2) : null) : null
	...
	return (
		<div className="box_container" style={{ margin: '0 0 0 0.5rem' }}>
			<div className="box" style={{ width: width[0] }}>
				<input type="checkbox" checked={props.checked} onChange={(e) => props.onChange()} />
			</div>
			<div className="box" style={{ width: width[1] }} onClick={() => props.setEntry(data)}>
				{data._id}
			</div>
			<div className="box" style={{ width: width[2] }}>
				<img src={data && data.image && data.image[0] && data.image[0].replace('original', 'small')} />
			</div>
