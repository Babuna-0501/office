...
const List = (props) => {
	...
	products.map((data) => {
		...
		if (foo) {
			...
			renderHTML.push(<Entry data={data} checked={ids.includes(data._id)} onChange={() => onChange(data._id)} productGroups={props.productGroups} setEntry={props.setEntry} key={Math.random()} allProducts={whcontext.allProducts} />)
		}
	})
			...
			<div id="container-list" style={{ margin: '0 0 0 1rem' }}>
				<div className="blah margintop8px" style={{ height: '60px' }}>
					<div className="blahblah">
						<div className="width30px" style={{ display: 'flex', alignItems: 'center' }} >
							<input type="checkbox" checked={products.length > 0 && ids.length == products.length} onClick={(e) => selectAll(e.target.checked)} />
						</div>
					</div>
					<div className="blahblah">
						<div className={`width80px`} style={{ padding: '0 1rem 0 0' }}>
							<p className="blahblahheader">ID</p>
							<input type="text" className={css.header_searchinput} onKeyUp={(e) => setId(e.target.value)} />
						</div>
					</div>
					<div className="blahblah">
						<div className={`width340px`} style={{ padding: '0 1rem 0 0' }}>
							<p className="blahblahheader">Бүтээгдэхүүний нэр</p>
							<input type="text" className={css.header_searchinput} onKeyUp={(e) => setName(e.target.value)} />
						</div>
					</div>
					<div className="blahblah">
						<div className={`width160px`} style={{ padding: '0 1rem 0 0' }}>
							<p className="blahblahheader">Баркод</p>
							<input type="text" className={css.header_searchinput} onKeyUp={(e) => setBarcode(e.target.value)} />
						</div>
					</div>
					<div className={`width180px`}></div>
					<div className="blahblah">
						<div className={`width140px`} style={{ padding: '0 1rem 0 0' }}>
							<p className="blahblahheader">Үлдэгдэл</p>
							<input type="text" className={css.header_searchinput} onKeyUp={(e) => setStock(e.target.value)} />
						</div>
					</div>
