...
const Orlogo = props => {
  ...
  const fetchData = () => {
    const url = `${process.env.REACT_APP_API_URL2}/api/shipment?type=${
      parseInt(companyId) === 14005 ? '3' : '2'
    }&from=${
      props.wh
    }&startDate=${startDate}&endDate=${endDate}&products=true&createdDate=true`;
    var requestOptions = {
      method: 'GET',
      headers: myHeaders,
      redirect: 'follow'
