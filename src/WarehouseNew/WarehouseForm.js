...
const WarehouseForm = (props) => {
	...
	const save = () => {
		if(saving === false) {
			const borderColor = document.getElementById('warehouseName').style.borderColor
		    ...
		    const url = `https://api2.ebazaar.mn/warehouse`
			...
			<div id="containerFloatPage">
				<div id="floatPage">
					<div className="pageHeader" id="pageHeader">
						...
						<span className="pageClose" onClick={() => props.setWarehouseForm()}><img src="https://admin.ebazaar.mn/images/close.svg" alt="" /></span>
					</div>
					<div className="main">
						<div>
							<label>Агуулахын нэр</label>
							<input type="text" placeholder="Агуулахын нэр оруулна уу" id="warehouseName" />
						</div>
						<div>
							<label>Агуулахын байршил</label>
							<input type="text" placeholder="Агуулахын байршил оруулна уу" id="warehouseLocation" />
						</div>
						<div>
					...
					<div id="pageBottom">
						<button onClick={() => save()} disabled={saving ? true : false} className="pageButton">Агуулах үүсгэх</button>
					</div>
				</div>
			</div>
