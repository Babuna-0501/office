...
const Brands = () => {
  ...
  const fetchBrand = () => {
    ...
    fetch(
      // `${process.env.REACT_APP_API_URL2}/api/brands?search=${searchValue}`,
      `${process.env.REACT_APP_API_URL2}/brand?search=${searchValue}`,
      requestOptions
    )
      .then(r => r.json())
      .then(res => {
    ...
    // fetch(`${process.env.REACT_APP_API_URL2}/api/backoffice/suppliers`, requestOptions)
    // 	.then(r => r.json())
    // 	.then(res => {
