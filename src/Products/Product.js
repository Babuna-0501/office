...
const Product = props => {
  ...
  if (product && product.image) {
    image = product?.image[0] && replaceImageUrl(product.image[0]);
  } else {
  ...
  const Butarhai = () => {
    ...
    let urlNew = `${process.env.REACT_APP_API_URL2}/api/product/measure`;
    fetch(urlNew, requestOptions)
      .then(response => response.json())
      .then(result => {
  ...
  const sync = () => {
    ...
    let urlNew = `${process.env.REACT_APP_API_URL2}/api/pickpack/createproduct`;
    fetch(urlNew, requestOptions)
      .then(response => response.json())
      .then(result => {
        if (result.code === 200) {
  ...
  const deleteHandler = product => {
    ...
    fetch(
      `${process.env.REACT_APP_API_URL2}/product/remove?productId=${product._id}`,
      requestOptions
    )
      .then(res => {
  ...
  return (
    <div>
      ...
      >
          ...
          <img
            src={
              image && image !== 'https://ebazaar.mn/icon/photo-add.svg'
                ? replaceImageUrl(image.replace('original', 'original'))
        ...
        {props.userData.company_id === '|1|' && (
          <div style={{ width: '120px' }}>
            {product.thirdparty_data?.pickpack?.sku ?? ''}
          </div>
        )}
        ...
        {pathname !== '/oresh' ? (
          <>
            ...
            <div style={{ width: '100px' }}>
              <span>
                {product.city_tax === 1 ? (
                  <img src='/media/on.svg' alt='small map' />
                ) : (
                  <img src='/media/off.svg' alt='small map' />
                )}
              </span>
            </div>
            {/* <div style={{ width: "120px" }}>
