...
const List = (props) => {
  ...
  const getProducts = async () => {
    ...
    let urlNew = "";
    let params = "";
    // let urlNew = `https://api2.ebazaar.mn/api/products/get1?supplier=${suppID}&search=${searchValue}&bar_code=${bar_code}&sku=${sku}&product_measure=${productMeasure}&page=${page}&limit=50&id=${productID}`;
    // let urlNew = `https://api2.ebazaar.mn/api/products/get1?page=${page}&limit=50`;
    ...
    if (
      ...
      // urlNew = `https://api2.ebazaar.mn/api/products/get1?vendor=${props.userData.company_id.replaceAll(
      //   "|",
      //   ""
    ...
    urlNew = `https://api2.ebazaar.mn/api/products/get1?${params}&page=${page}&limit=50`;
    ...
    /*if (warehouseall && warehouseall.length > 0) {
      ...
      const warehouse = () => {
        const warehouseIds = warehouseall
          ...
          .join(",");
        const url = `https://api2.ebazaar.mn/api/warehouse?ids=${warehouseIds}&allProducts=true`;
  ...
  const getZones = async () => {
    ...
    let urlNew = `https://api2.ebazaar.mn/api/zones`;
    await fetch(urlNew, requestOptions)
      .then((r) => r.json())
      .then((response) => {
