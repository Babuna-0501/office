...
const VatDetailForm = props => {
  ...
  let buttonHTML = returning ? (
    ...
    <>
      <button
        ...
        onClick={() => props.setVatDetail(false)}
      >
        Хаах
      </button>
      <button className='button primary large' onClick={() => returnReceipt()}>
        Баримт буцаах
      </button>
    </>
  );
  const returnReceipt = () => {
    ...
    let url = `${process.env.REACT_APP_API_URL2}/pos-api/receipt/return`;
    fetch(url, requestOptions)
      ...
      .then(response => {
        if (response.message === 201 && response.data === 'SUCCESS') {
          ...
          url = `${process.env.REACT_APP_API_URL2}/api/shipment/config`;
          requestOptions = {
            method: 'PATCH',
            headers: myHeaders,
            redirect: 'follow',
          ...
          fetch(url, requestOptions)
            ...
            .then(response => {
              ...
              document.getElementById('billId' + data.billId).style.color =
                'red';
            });
        } else {
          alert('Алдаа гарлаа.');
  ...
  const registerInfo =
    data.taxPayerType !== 'individual' ? (
      <>
        <div>
          <span
            className='width300px inlineblock marginbottom1rem'
