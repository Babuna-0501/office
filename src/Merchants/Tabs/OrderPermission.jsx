...
export const OrderPermission = (props) => {
  ...
  useEffect(() => {
    getSupplier();
    fetch(`https://api2.ebazaar.mn/users/alldata?page=1&limit=1&users=${userId}`, requestOptions)
      .then((r) => r.json())
      .then((res) => {
  ...
  useEffect(() => {
    if (includedSuppId.length > 0) {
      fetch(`https://api2.ebazaar.mn/api/backoffice/suppliers?id=${includedSuppId}`, requestOptions)
        .then((r) => r.json())
        .then((res) => {
  ...
  return (
    <div className={css.container}>
      ...
      {(update ? updatedPermission?.length > 3 : orderEditPermission?.length > 3) ? (
        ...
        <div style={{ textAlign: "center", marginTop: "20px" }}>Эрх өгсөн нийлүүлэгч алга</div>
      )}
