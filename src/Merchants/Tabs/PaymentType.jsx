...
export const PaymentType = ({ id, permission, fetchdata }) => {
  ...
  useEffect(() => {
    if (searchValue.length >= 3 || searchValue.length === 0) {
      fetch(`https://api2.ebazaar.mn/api/backoffice/suppliers?name=${searchValue}`, requestOptions)
        .then((r) => r.json())
        .then((res) => {
  ...
  const PutRequest = () => {
    ...
    fetch(`https://api2.ebazaar.mn/tradeshops?tradeshopId=${id}`, requestOptions)
      .then((res) => res.json())
      .then((response) => {
