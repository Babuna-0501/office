...
function Detail2(props) {
  ...
  const getNewData = () => {
    ...
    fetch(
      `https://api2.ebazaar.mn/users/alldata?page=1&limit=1&userId=${props.data.user_id}`,
      requestOptions
    )
      .then((res) => res.json())
      .then((response) => {
  ...
  const fetchdata = async () => {
    ...
    console.log(
      " `https://api2.ebazaar.mn/api/tradeshop/alldata?tradeshop_id=${props.data.tradeshop_id}`,",
      `https://api2.ebazaar.mn/api/tradeshop/alldata?tradeshop_id=${props.data.tradeshop_id}`
    );
  ...
  useEffect(() => {
    if (userResult && locations) {
      ...
      setAllData((prev) => ({
        ...prev,
        c_ANname: [].concat(...userData).filter((e) => e.TradeshopID === props.data.tradeshop_id)[0]
          .TradeshopName,
        ...
        city: locations?.find((e) => e.location_id === parseInt(userResult?.t_city))?.location_id,
        district: locations?.find((e) => e.location_id === parseInt(userResult?.t_disctrict))
          ?.location_id,
        khoroo: locations?.find((e) => e.location_id === parseInt(userResult?.t_khoroo))
          ?.location_id,
  ...
  useEffect(() => {
    if (props.user.company_id !== "|1|") {
      setAllData({
        ...
        city: locations?.find((e) => e.location_id === parseInt(props.data.city))?.location_id,
        district: locations?.find((e) => e.location_id === parseInt(props.data.district))
          ?.location_id,
        khoroo: locations?.find((e) => e.location_id === parseInt(props.data.horoo))?.location_id,
  ...
  const getSupplier = () => {
    ...
    fetch(url, requestOptions)
      ...
      .then((res) => {
        ...
        if (res.data[0].includeSupplierList?.length > 0 && res.data[0].includeSupplierList !== "") {
    ...
    var raw = {
      tradeshop: {
        ...
        Survey: JSON.stringify({
          ...
          numberOfHorizontalFreezers: allData?.t_survey?.numberOfHorizontalFreezers,
