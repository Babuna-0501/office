...
const FormInputs = (props) => {
	...
	const [mustHave, setMustHave] = useState(props.product !== 'new' && product.attributes['musthave'] === true ? true : false)
	useEffect(() => {
	...
	async function save() {
		...
		if(name.value !== '') {
			...
			props.save(name.value, barcode, country, manufacturer, vendor, category, sku, description, attributes)
		} else {
	...
	const update = (id) => {
		...
		if(name.value !== '') {
			...
			props.update(id, name.value, barcode, country, manufacturer, vendor, category, condition, storage, form, sku, boditsavlalt, zardagsavlalt, description)
		} else {
			const borderColor = name.style.borderColor
			name.style.borderColor = 'red'
			name.focus()
				...
				<div id="pageBody">
					<div className="left" style={{width: '400px'}}>
						<div className="inputContainer">
							...
							<input type="text" id="name" defaultValue={props.product === 'new' ? null : product.name} />
						</div>
						<div className="inputContainer">
							<label>Баркод:</label>
							<input type="text" id="barcode" defaultValue={props.product === 'new' ? null : product.bar_code} />
						</div>
						<div className="inputContainer">
						...
						<div className="inputContainer">
							<label>SKU:</label>
							<input type="text" id="sku" value={props.product === 'new' ? null : product.sku} />
						</div>
						<div className="inputContainer">
							<label>Бодит савлалт:</label>
							<input type="text" id="boditsavlalt" defaultValue={props.product !== 'new' && props.product.attributes && props.product.attributes['boxing'] && props.product.attributes['boxing']['box'] ? props.product.attributes['boxing']['box'] : ''} />
						</div>
						<div className="inputContainer">
							<label>Зардаг савлалт:</label>
							<input type="text" id="zardagsavlalt" defaultValue={props.product !== 'new' && props.product.attributes['boxing'] && props.product.attributes['boxing']['sell'] ? props.product.attributes['boxing']['sell'] : ''} />
						</div>
						<div className="inputContainer">
							<div>
								<div style={{display: 'flex', alignItems: 'center'}}>
								<input type="checkbox" style={{width: '20px', height: '20px'}} id="musthave" checked={mustHave} onChange={(e) => setMustHave(mustHave === true ? false : true)} />
								<label for="musthave" style={{cursor: 'pointer'}}>Заавал байх</label></div>
							</div>
						</div>
						<div className="inputContainer">
					...
					<div className="right" style={{left: '400px'}}>
						...
						<textarea className="text margintop1rem" id="description">{props.product.description}</textarea>
						<div className="containerButtons">
							<button className="button primary large" onClick={() => props.product === 'new' ? save() : update(product._id)}>{props.product === 'new' ? 'Бүтээгдэхүүнийг бүртгэх' : 'Өөрчлөлтийг хадгалах'}</button>
						</div>
					</div>
				</div>
			</div>
