...
const Index = () => {
  ...
  const fetchData = () => {
    ...
    const url = `${process.env.REACT_APP_API_URL2}/api/merchants?page=1`;
  ...
  return (
    <>
      ...
      <div className='containerPageButtons'>
        <button className='pageButton' onClick={() => setCustomer('new')}>
          + Шинэ харилцагч
        </button>
        <button className='pageButton'>Харилцагч масс импортлох</button>
        <button className='pageButton' onClick={() => downloadReport(foo)}>
          Харилцагчийн тайлан
        </button>
      </div>
      {data ? (
